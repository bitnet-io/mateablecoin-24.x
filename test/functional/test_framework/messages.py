...
# Copyright (c) 2010-2021 The Mateable Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Mateable test framework primitive and message structures
...
# Objects that map to mateabled objects, which can be serialized/deserialized
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += struct.pack("<i", 0)  # Mateable Core ignores version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in mateabled
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in mateabled
        try:
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in mateabled indicates these should be deserialized as blocks
