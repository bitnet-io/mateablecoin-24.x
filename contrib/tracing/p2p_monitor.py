...
""" Interactive mateabled P2P network traffic monitor utilizing USDT and the
    net:inbound_message and net:outbound_message tracepoints. """

# This script demonstrates what USDT for Mateable Core can enable. It uses BCC
...
def main(mateabled_path):
    peers = dict()

    mateabled_with_usdts = USDT(path=str(mateabled_path))

    # attaching the trace functions defined in the BPF program to the tracepoints
    mateabled_with_usdts.enable_probe(
        probe="inbound_message", fn_name="trace_inbound_message")
    mateabled_with_usdts.enable_probe(
        probe="outbound_message", fn_name="trace_outbound_message")
    bpf = BPF(text=program, usdt_contexts=[mateabled_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE:", sys.argv[0], "path/to/mateabled")
        exit()
    path = sys.argv[1]
    main(path)
