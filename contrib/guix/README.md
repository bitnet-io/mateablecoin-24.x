# Bootstrappable Mateable Core Builds

This directory contains the files necessary to perform bootstrappable Mateable
Core builds.
...
To build Mateable Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Mateable Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/mateable-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/mateable-core/guix.sigs).
