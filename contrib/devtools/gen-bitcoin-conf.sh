...
MATEABLED=${MATEABLED:-$BINDIR/mateabled}
...
[ ! -x "$MATEABLED" ] && echo "$MATEABLED not found or not executable." && exit 1

DIRTY=""
VERSION_OUTPUT=$($MATEABLED --version)
if [[ $VERSION_OUTPUT == *"dirty"* ]]; then
  DIRTY="${DIRTY}${MATEABLED}\n"
fi

if [ -n "$DIRTY" ]
then
  echo -e "WARNING: $MATEABLED was built from a dirty tree.\n"
  echo -e "To safely generate a bitcoin.conf file, please commit your changes to $MATEABLED, rebuild, then run this script again.\n"
fi
...
## Generated by contrib/devtools/gen-mateable-conf.sh.
##
...
# parse the output from mateabled --help
