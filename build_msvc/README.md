Building Mateable Core with Visual Studio
========================================

Introduction
---------------------
Visual Studio 2022 is minimum required to build Mateable Core.

Solution and project files to build with `msbuild` or Visual Studio can be found in the `build_msvc` directory.

To build Mateable Core from the command-line, it is sufficient to only install the Visual Studio Build Tools component.
...
Qt
---------------------
To build Mateable Core with the GUI, a static build of Qt is required.
...
To build Mateable Core without Qt, unload or disable the `mateable-qt`, `libbitcoin_qt` and `test_mateable-qt` projects.
...
Security
---------------------
[Base address randomization](https://docs.microsoft.com/en-us/cpp/build/reference/dynamicbase-use-address-space-layout-randomization?view=msvc-160) is used to make Mateable Core more secure. When building Mateable using the `build_msvc` process base address randomization can be disabled by editing `common.init.vcproj` to change `RandomizedBaseAddress` from `true` to `false` and then rebuilding the project.

To check if `mateabled` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/mateabled.exe
```
