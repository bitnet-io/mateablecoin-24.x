...
static void SetupMateableTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " mateable-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  mateable-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoin transaction\n"
                "or:     mateable-tx [options] -create [commands]   Create hex-encoded bitcoin transaction\n"
                "\n";
