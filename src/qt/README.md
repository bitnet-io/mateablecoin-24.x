This directory contains the source code for the Mateable Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Mateable UI.
...
* UI elements like MateableAmountField, which inherit from QWidget.
...
4. Enter `mateable-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `mateable-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `mateable-qt`
9. You're all set! Start developing, building, and debugging the Mateable Core GUI
