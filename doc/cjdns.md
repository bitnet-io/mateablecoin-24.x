# CJDNS support in Mateable Core

It is possible to run Mateable Core over CJDNS, an encrypted IPv6 network that
...
done outside of Mateable Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Mateable network and individual nodes.
...
before it will work with your Mateable Core node. This is described in steps
...
## Run Mateable Core with CJDNS

Once you are connected to the CJDNS network, the following Mateable Core
...
When enabled, this option tells Mateable Core that it is running in an
...
IPv6 local network. This helps Mateable Core perform better address management:
...
CJDNS support was added to Mateable Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `mateable-cli -addrinfo` to see the
...
## CJDNS-related information in Mateable Core

There are several ways to see your CJDNS address in Mateable Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `mateable-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `mateable-cli getpeerinfo`).
