# Fuzzing Mateable Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Mateable Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
## Overview of Mateable Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/bitcoin_report.pdf) includes an external overview of the status of Mateable Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.
...
It is possible to specify `mateabled` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`mateable-core/qa-assets`](https://github.com/mateable-core/qa-assets) repo.

To fuzz `process_message` using the [`mateable-core/qa-assets`](https://github.com/mateable-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/mateable-core/qa-assets
...
Fuzzing on a harness compiled with `--with-sanitizers=address,fuzzer,undefined` is good for finding bugs. However, the very slow execution even under libFuzzer will limit the ability to find new coverage. A good approach is to perform occasional long runs without the additional bug-detectors (configure `--with-sanitizers=fuzzer`) and then merge new inputs into a corpus as described in the qa-assets repo (https://github.com/mateable-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).  Patience is useful; even with improved throughput, libFuzzer may need days and 10s of millions of executions to reach deep/hard targets.
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`mateable-core/qa-assets`](https://github.com/mateable-core/qa-assets) repo.

Every single pull request submitted against the Mateable Core repo is automatically tested against all inputs in the [`mateable-core/qa-assets`](https://github.com/mateable-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Mateable Core more robust.
...
with certain assembly code from Mateable Core's code. See [developer notes on sanitizers](https://github.com/bitcoin/bitcoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Mateable Core using afl++

## Quickstart guide

To quickly get started fuzzing Mateable Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Mateable Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Mateable Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Mateable Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Mateable
Core without having to write any custom fuzzing harness. The `mateabled` server
...
```sh
$ mkdir mateable-honggfuzz-p2p/
$ cd mateable-honggfuzz-p2p/
$ git clone https://github.com/bitcoin/bitcoin
$ cd bitcoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/mateabled.cpp b/src/mateabled.cpp
...
EOF
$ make -C src/ mateabled
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/mateabled -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:18444 -logthreadnames \
                       -debug
```

# Fuzzing Mateable Core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing Mateable Core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
Mateable Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/mateable-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=mateable-core).
...
because Mateable is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_mateable-core/latest).
