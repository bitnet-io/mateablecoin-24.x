...
- [Development guidelines](#development-guidelines)
    - [General Mateable Core](#general-mateable-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Mateable Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while mateabled is running with the `logging`
RPC.  For instance, launching mateabled with `-debug` or `-debug=1` will turn on
...
Mateable Core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run mateabled.

You can then use the `-debug=lock` configuration option at mateabled startup or
`mateable-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `mateable-cli logging [] '["lock"]'`.
...
$ valgrind -v --leak-check=full src/mateabled -printtoconsole
...
To profile a running mateabled process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep mateabled` -- sleep 60
```
...
Mateable Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`mateabled`)](https://doxygen.bitcoincore.org/mateabled_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `mateabled.cpp`. Responsible for starting up and
...
However, in open source software such as Mateable Core, where everyone uses
...
pay attention to for reviewers of Mateable Core code.

General Mateable Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Mateable Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `MATEABLE_FOO_BAR_H`, e.g.

```c++
#ifndef MATEABLE_FOO_BAR_H
#define MATEABLE_FOO_BAR_H
...
#endif // MATEABLE_FOO_BAR_H
```
...
Some of these are maintained by active developers of Mateable Core, in which case
...
a Mateable Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- src/leveldb
  - Subtree at https://github.com/mateable-core/leveldb-subtree ; maintained by Core contributors.
...
- src/crc32c
  ...
  - Subtree at https://github.com/mateable-core/crc32c-subtree ; maintained by Core contributors.
  - Upstream at https://github.com/google/crc32c ; maintained by Google.

- src/secp256k1
  - Upstream at https://github.com/mateable-core/secp256k1/ ; maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/mateable-core/ctaes ; maintained by Core contributors.
...
file descriptors, it will cause problems with Mateable's `select()` loop, because
...
compatibility between nodes. This happened in Mateable 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Mateable's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `mateable-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `mateable-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Mateable Core, the wallet was always
