...
The data directory is the default location where the Mateable Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.bitcoin/`
macOS    | `$HOME/Library/Application Support/Mateable/`
Windows  | `%APPDATA%\Mateable\` <sup>[\[1\]](#note1)</sup>
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Mateable blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `bitcoin.conf`        | User-defined [configuration settings](mateable-conf.md) for `mateabled` or `mateable-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `mateabled.pid`        | Stores the process ID (PID) of `mateabled` or `mateable-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `mateabled` or `mateable-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [bitcoin.conf](mateable-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`mateable-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Mateable Core:
