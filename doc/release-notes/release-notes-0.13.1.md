Mateable Core version 0.13.1 is now available from:

  <https://bitcoin.org/bin/mateable-core-0.13.1/>
...
In addition to that, with 0.12.x there have been varied reports of Mateable Core
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Mateable wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Mateable.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Mateable block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Mateable addresses (both P2PKH addresses
  ...
  believed to be provided by Mateable's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Mateable paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Mateable's current ledger (called the UTXO set),
  ...
  note that Mateable Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Mateable users to individually opt-in to almost any change in the Mateable
  Script language when those users receive new transactions.  Features
  currently being researched by Mateable Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Mateable community members are studying several other improvements
...
Since Mateable Core 0.10.0, nodes have defaulted to only relaying and
...
- `mateable-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `mateable-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- #8829 `2a8bca4` Add mateable-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` mateable-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_mateableds() into stop_nodes() (MarcoFalke)
...
- #8817 `bcf3806` update mateable-tx to output witness data (jnewbery)
