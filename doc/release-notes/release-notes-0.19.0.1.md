Mateable Core version 0.19.0.1 is now available from:

  <https://bitcoincore.org/bin/mateable-core-0.19.0.1/>
...
Mateable Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Mateable Core on unsupported systems.
...
to open the Mateable Core .dmg. This is due to new signing requirements
imposed by Apple, which the Mateable Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin/bitcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Mateable Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Mateable
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `mateabled -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Mateable Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Mateable Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Mateable Core. (#14054)

- To eliminate well-known denial-of-service vectors in Mateable Core,
  ...
  This prevents Mateable Core from sending the BIP111 NODE_BLOOM service
  ...
  of Mateable Core and nodes that have manually enabled BIP37 support,
...
- By default, Mateable Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `mateable-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Mateable Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Mateable Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Mateable Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Mateable address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Mateable Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Mateable AEAD (jonasschnelli)
...
- #15639 mateable-wallet tool: Drop libbitcoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in mateabled/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "mateable-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of mateabled build (NicolasDorier)
...
- #15773 Add MateableTestFramework::sync_* methods (MarcoFalke)
...
- #15896 feature_filelock, interface_bitcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Mateable Core (luke-jr)
...
- #16489 log: harmonize mateabled logging (jonatack)
...
- #15683 Comment for seemingly duplicate LIBMATEABLE_SERVER (Bushstar)
...
- #16863 Add a missing closing parenthesis in the mateable-wallet's help (darosior)
...
- #16912 Remove Doxygen intro from src/mateabled.cpp (ch4ot1c)
