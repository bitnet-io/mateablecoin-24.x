...
Mateable Core version 0.20.0 is now available from:

  <https://bitcoincore.org/bin/mateable-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Mateable
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Mateable Core on
unsupported systems.

From Mateable Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Mateable Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `MATEABLE_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Mateable Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Mateable Core version 0.18.0.
...
- Testing or debugging of implementations of the Mateable P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Mateable Core. Mateable Core logs debug messages
...
- OpenSSL is no longer used by Mateable Core.  (#17265)

- BIP70 support has been fully removed from Mateable Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  `mateabled help` for more information.  This option is experimental and
...
- All custom settings configured when Mateable Core starts are now
...
- The "Start Mateable Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Mateable Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Mateable Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/mateable-core/HWI) for
  signing.  Future versions of Mateable Core should support a GUI option
...
- Mateable Core's automatically-generated source code documentation is
...
- The `mateable-cli` utility used with the `-getinfo` parameter now
...
- The `mateable-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17741 Included `test_mateable-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBMATEABLE_SERVER` linking (fanquake)
...
- #18429 Remove double `LIBMATEABLE_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for mateable-cli on Windows (fanquake)
...
- #18166 ci: Run fuzz testing test cases (mateable-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone mateable-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Mateable Core files (hebasto)
...
- #17119 Fix broken mateable-cli examples (andrewtoth)
...
- #17503 Remove bitness from mateable-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to mateable-wallet (hel-o)
...
- #16115 On mateabled startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from mateable-cli -getinfo (malevolent)
