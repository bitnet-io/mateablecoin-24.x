Mateable Core version 0.13.0 is now available from:

  <https://bitcoin.org/bin/mateable-core-0.13.0/>
...
mateable-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/mateable-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Mateable Core code base has
...
HD wallets are incompatible with older versions of Mateable Core.
...
proposed parameters, a future release of Mateable Core is expected that
...
The two phases are distinct in the Mateable-Qt GUI. During the first one,
...
- #7550 `8b958ab` Input-from-stdin mode for mateable-cli (laanwj)
...
- #7613 `3798e5d` Add autocomplete to mateable-qt's console window (GamerSg)
...
- #8256 `af2421c` BUG: mateable-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from mateabled (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the Mateable-Core repository and website to the About Dialog (MarcoFalke)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for mateabled procs (MarcoFalke)
...
- #8164 `0f24eaf` [Mateable-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of mateabled startup (laanwj)
...
- #7975 `fc23fee` Update mateable-core GitHub links (MarcoFalke)
