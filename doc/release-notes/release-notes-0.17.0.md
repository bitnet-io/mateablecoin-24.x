Mateable Core version 0.17.0 is now available from:

  <https://bitcoincore.org/bin/mateable-core-0.17.0/>
...
prefixing the option with the network, such as:
    ...
    test.uacomment=mateable-testnet
...
be used in V0.17 by starting mateabled with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Mateable Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Mateable transactions that are not fully signed
...
Overall, the construction of a fully signed Mateable transaction goes through the
following steps:
...
- An **Extractor** produces a valid Mateable transaction (in network format)
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart mateabled with
...
- When running mateabled with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If mateabled
...
- #13528 `b877c39` Move MateableGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libmateable-util to libmateable-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for mateable-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling mateable-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use MateableTestFramework (jnewbery)
...
- #11772 `0630974` Change invalidblockrequest to use MateableTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use MateableTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use MateableTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for MateableTestFramework (WillAyd)
...
- #12102 `7fb8fb4` Apply hardening measures in mateabled systemd service file (Flowdalic)
- #12652 `55f490a` mateable-cli: Provide a better error message when mateabled is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to mateabled in debian package (ghost)
...
- #13603 `453ae5e` mateable-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix mateable-cli --version (Empact)
...
- #12877 `2b54155` Use mateabled in Tor documentation (knoxcard)
...
- #13346 `60f0358` update mateable-dot-org links in release-process.md (fanquake)
...
- #13905 `73a09b4` fixed mateable-cli -help output for help2man (hebasto)
