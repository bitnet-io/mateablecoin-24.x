...
Mateable Core version 23.0 is now available from:

  <https://bitcoincore.org/bin/mateable-core-23.0/>
...
using the Linux kernel, macOS 10.15+, and Windows 7 and newer.  Mateable
...
- A mateabled node will no longer rumour addresses to inbound peers by default.
  They will become eligible for address gossip after sending an ADDR, ADDRV2,
  or GETADDR message. (#21528)

- Before this release, Mateable Core had a strong preference to try to connect only to peers that listen on port 8333. As a result of that, Mateable nodes listening on non-standard ports would likely not get any Mateable Core peers connecting to them. This preference has been removed. (#23542)
...
Mateable Core release binaries for Linux now include experimental tracepoints which
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. Mateable Core
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from Mateable Core in 22.0. (#22544)
