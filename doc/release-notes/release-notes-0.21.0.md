...
Mateable Core version 0.21.0 is now available from:

  <https://bitcoincore.org/bin/mateable-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by Mateable Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Mateable Core will no longer
...
- A new `mateable-cli -netinfo` command provides a network peer connections
...
- A new `mateable-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `mateable-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `mateable-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Mateable Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `mateable-wallet` tool which performs the salvage
...
Mateable Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`bitcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Mateable Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `mateable-cli createwallet` or `mateable-wallet
...
Users who already have a Mateable Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Mateable Core.
...
The change to using descriptors changes the default derivation paths used by Mateable Core
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Mateable Core is
...
- #20198 Show name, format and if uses descriptors in mateable-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add mateable-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in mateable-cli (laanwj)
...
### Build system
- #18504 Drop mateable-tx and mateable-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in MATEABLE_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for mateable-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Mateable Core using Honggfuzz (practicalswift)
...
- #18817 Document differences in mateabled and mateable-qt locale handling (practicalswift)
