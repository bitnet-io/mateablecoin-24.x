Mateable Core version 0.18.0 is now available from:

  <https://bitcoincore.org/bin/mateable-core-0.18.0/>
...
`/Applications/Mateable-Qt` (on Mac) or `mateabled`/`mateable-qt` (on
Linux).
...
recommended to use Mateable Core on unsupported systems.
...
10.10. Additionally, Mateable Core does not yet change appearance when
...
`contrib/init/mateabled.service`, it has been changed to use
`/var/lib/mateabled` as the data directory instead of
`~bitcoin/.bitcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/mateabled` will
exist has enough space (check using `df -h /var/lib/mateabled`), and
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Mateable Core
...
The systemd init file (`contrib/init/mateabled.service`) has been changed
to use `/var/lib/mateabled` as the data directory instead of
`~bitcoin/.bitcoin`. This change makes Mateable Core more consistent with
...
- A new [document](https://github.com/bitcoin/bitcoin/blob/master/doc/mateable-conf.md)
  about the `bitcoin.conf` file describes how to use it to configure
  Mateable Core.

- A new document introduces Mateable Core's BIP174 [Partially-Signed
  Mateable Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Mateable-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Mateable Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Mateable Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Mateable Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart mateabled with the `-deprecatedrpc=generate`
...
- A new `mateable-wallet` tool is now distributed alongside Mateable
...
This section describes planned changes to Mateable Core that may affect
other Mateable software and services.

- Since version 0.16.0, Mateable Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Mateable Core 0.20 (expected about
  a year after 0.18), Mateable Core will default to native segwit
  ...
  to bech32 addresses, and if the Mateable Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Mateable Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Mateable Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Mateable Core
...
- A sub-project of Mateable Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Mateable Core.  See their [project
  page](https://github.com/mateable-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Mateable Core's own implementation, although entropy
  gathered by Mateable Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Mateable Core a
...
- On macOS, Mateable Core now opts out of application CPU throttling
...
- #13926 [Tools] mateable-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like mateabled (MarcoFalke)
...
- #14521 qt, docs: Fix `mateable-qt -version` output formatting (hebasto)
...
- #11625 Add MateableApplication & RPCConsole tests (ryanofsky)
...
- #15154 Configure: mateable-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/mateable-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Mateable Core (sipsorcery)
...
- #13953 Fix deprecation in mateable-util-test.py (isghe)
...
- #12775 Integration of property based testing into Mateable Core (Christewart)
...
- #12246 Bugfix: Only run mateable-tx tests when mateable-tx is enabled (luke-jr)
...
- #14186 mateable-cli: don't translate command line options (HashUnlimited)
...
- #14497 Add doc/mateable-conf.md (hebasto)
...
- #15354 Add missing `mateable-wallet` tool manpages (MarcoFalke)
