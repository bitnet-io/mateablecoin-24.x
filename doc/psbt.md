# PSBT Howto for Mateable Core

Since Mateable Core 0.17, an RPC interface exists for Partially Signed Mateable
...
PSBT is an interchange format for Mateable transactions that are not fully signed
...
#### Multisig with multiple Mateable Core instances
...
Mateable Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `mateable-cli` is used.
