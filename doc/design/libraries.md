...
| Name                     | Description |
|--------------------------|-------------|
| *libbitcoin_cli*         | RPC client functionality used by *mateable-cli* executable |
...
| *libbitcoinqt*           | GUI functionality used by *mateable-qt* and *mateable-gui* executables |
| *libbitcoin_ipc*         | IPC functionality used by *mateable-node*, *mateable-wallet*, *mateable-gui* executables to communicate when [`--enable-multiprocess`](multiprocess.md) is used. |
| *libbitcoin_node*        | P2P and RPC server functionality used by *mateabled* and *mateable-qt* executables. |
| *libbitcoin_util*        | Home for common functionality shared by different executables and libraries. Similar to *libbitcoin_common*, but lower-level (see [Dependencies](#dependencies)). |
| *libbitcoin_wallet*      | Wallet functionality used by *mateabled* and *mateable-wallet* executables. |
| *libbitcoin_wallet_tool* | Lower-level wallet functionality used by *mateable-wallet* executable. |
| *libbitcoin_zmq*         | [ZeroMQ](../zmq.md) functionality used by *mateabled* and *mateable-qt* executables. |
...
mateable-cli[mateable-cli]-->libbitcoin_cli;

mateabled[mateabled]-->libbitcoin_node;
mateabled[mateabled]-->libbitcoin_wallet;

mateable-qt[mateable-qt]-->libbitcoin_node;
mateable-qt[mateable-qt]-->libbitcoinqt;
mateable-qt[mateable-qt]-->libbitcoin_wallet;

mateable-wallet[mateable-wallet]-->libbitcoin_wallet;
mateable-wallet[mateable-wallet]-->libbitcoin_wallet_tool;
...
class mateable-qt,mateabled,mateable-cli,mateable-wallet bold
```
</td></tr><tr><td>
...
- *libbitcoin_common* should serve a similar function as *libbitcoin_util* and be a place for miscellaneous code used by various daemon, GUI, and CLI applications and libraries to live. It should not depend on anything other than *libbitcoin_util* and *libbitcoin_consensus*. The boundary between _util_ and _common_ is a little fuzzy but historically _util_ has been used for more generic, lower-level things like parsing hex, and _common_ has been used for mateable-specific, higher-level things like parsing base58. The difference between util and common is mostly important because *libbitcoin_kernel* is not supposed to depend on *libbitcoin_common*, only *libbitcoin_util*. In general, if it is ever unclear whether it is better to add code to *util* or *common*, it is probably better to add it to *common* unless it is very generically useful or useful particularly to include in the kernel.
