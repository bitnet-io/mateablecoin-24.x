# User-space, Statically Defined Tracing (USDT) for Mateable Core

Mateable Core includes statically defined tracepoints to allow for more
...
**Note**: Mateable Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Mateable Core
...
Multiple tools can list the available tracepoints in a `mateabled` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Mateable Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/mateabled
stap net        outbound_message 0x0000000000107c05 /src/mateabled
stap validation block_connected  0x00000000002fb10c /src/mateabled
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Mateable Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/mateabled | grep NT_STAPSDT -A 4 -B 2
